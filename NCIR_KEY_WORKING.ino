...
// Pin Definitions - CoreS3 with Key UNIT
#define KEY_PIN 8     // Key unit input pin (G9)
#define LED_PIN 9     // Key unit LED pin (G8)
#define NUM_LEDS 1
...
namespace Config {
    namespace Display {
        ...
        const unsigned long MIN_REFRESH_INTERVAL = 250;  // Minimum time between screen updates
        const float TEMP_UPDATE_THRESHOLD = 0.5f;       // Minimum temperature change to trigger update
...
public:
    ...
    bool ledActive = false;      // Track LED state
...
// Enhanced TempReading structure
struct TempReading {
    float temperature;
    bool valid;
    uint8_t errorCode;
...
void handleKeyAndLED() {
    ...
    if (!currentKeyState && lastKeyState) {  // Key just pressed
        Serial.println("Key pressed - Toggling monitoring state");
        state.keyPressed = true;
        state.isMonitoring = !state.isMonitoring;  // Toggle monitoring state
...
bool selectTemperatureUnit() {
    ...
    CoreS3.Display.drawString("C", celsiusBtn.x + celsiusBtn.width/2 + 4, celsiusBtn.y + celsiusBtn.height/2 + 4 - 10);
    CoreS3.Display.setTextColor(Config::Display::COLOR_TEXT);
    CoreS3.Display.drawString("C", celsiusBtn.x + celsiusBtn.width/2, celsiusBtn.y + celsiusBtn.height/2 - 10);
 
    // Draw F with 3D effect
    CoreS3.Display.setTextColor(0x666666);
    CoreS3.Display.drawString("F", fahrenheitBtn.x + fahrenheitBtn.width/2 + 4, fahrenheitBtn.y + fahrenheitBtn.height/2 + 4 - 10);
    CoreS3.Display.setTextColor(Config::Display::COLOR_TEXT);
    CoreS3.Display.drawString("F", fahrenheitBtn.x + fahrenheitBtn.width/2, fahrenheitBtn.y + fahrenheitBtn.height/2-10);
    Serial.println("Drew C/F labels");
    ...
    Serial.println("Drew descriptive text");
        ...
        if (CoreS3.Touch.getCount()) {
            ...
            if (touched.wasPressed()) {
                ...
                if (touchInButton(celsiusBtn, touched.x, touched.y)) {
                    Serial.println("Celsius selected");
                    // Highlight button
                    ...
                    Serial.println("Fahrenheit selected");
                    // Highlight button
    ...
    return true; // Default to Celsius if timeout
}
...
void updateDisplay() {
    ...
    if (state.currentScreen == SystemState::MAIN) {
        // Force redraw temperature by resetting lastDisplayedTemp
        state.lastDisplayedTemp = -999.0f;  // Add this line
        drawTemperature();
...
void handleTouch() {
    ...
    if (touch.y >= footerY) {
        ...
        if (touch.x >= Config::Display::MARGIN && 
            ...
            state.updateStatus(state.isMonitoring ? "Monitoring..." : "", 
                             Config::Display::COLOR_SUCCESS);
            state.lastDisplayedTemp = -999.0f;  // Force temperature redraw
            drawFooter();  // Update the footer buttons
            drawTemperature();  // Redraw temperature
            drawStatusBox();  // Update status
        }
        // Settings button
...
void setup() {
    ...
    auto cfg = M5.config();
    Serial.println("Initializing M5CoreS3...");
    CoreS3.begin(cfg);
    ...
    Serial.println("M5CoreS3 initialized");

    // Initialize KEY pin with pull-up
    Serial.println("Initializing KEY pin...");
    pinMode(KEY_PIN, INPUT_PULLUP);
    Serial.println("KEY pin initialized");
    
    // Initialize FastLED with proper error checking
    Serial.println("Initializing FastLED...");
    FastLED.addLeds<SK6812, LED_PIN, GRB>(leds, NUM_LEDS);  // Changed to SK6812 for Key UNIT
    FastLED.setBrightness(128);  // Increased brightness for better visibility
    leds[0] = CRGB::Black;      // Start with LED off
    FastLED.show();
    Serial.println("FastLED initialized");
    
    // Test KEY and LED
    Serial.println("Testing KEY and LED...");
    if (!digitalRead(KEY_PIN)) {
        Serial.println("KEY is pressed during startup!");
        leds[0] = CRGB::Red;     // Show red if key is pressed during startup
    } else {
        Serial.println("KEY is released during startup");
        leds[0] = CRGB::Green;   // Show green if key is released
    }
    FastLED.show();
    delay(1000);                 // Show the LED state for 1 second
    ...
    if (!selectTemperatureUnit()) {
        Serial.println("Temperature unit selection completed - Fahrenheit selected");
    } else {
        Serial.println("Temperature unit selection completed - Celsius selected");
    }
    ...
    // Test display by drawing a test pattern
    Serial.println("Drawing test pattern...");
    CoreS3.Display.fillScreen(Config::Display::COLOR_BACKGROUND);
    CoreS3.Display.drawRect(0, 0, CoreS3.Display.width(), CoreS3.Display.height(), Config::Display::COLOR_TEXT);
    CoreS3.Display.setTextSize(2);
    CoreS3.Display.setTextColor(Config::Display::COLOR_TEXT);
    CoreS3.Display.drawString("Display Test", CoreS3.Display.width()/2, CoreS3.Display.height()/2);
...
void loop() {
    ...
    // Handle KEY and LED states
    handleKeyAndLED();

    // Force update display periodically
    static unsigned long lastForceUpdate = 0;
    if (currentMillis - lastForceUpdate >= 5000) {  // Every 5 seconds
        updateDisplay();
        lastForceUpdate = currentMillis;
    }

    // Handle temperature updates with rate limiting
    if (state.isMonitoring && (currentMillis - state.lastTempUpdate >= Config::Temperature::DISPLAY_UPDATE_MS)) {
        TempReading tempReading = readTemperature();
        if (tempReading.valid) {
            state.currentTemp = tempReading.temperature;
            
            // Update display only when on main screen
            if (state.currentScreen == SystemState::MAIN) {
                drawTemperature();  // This will also draw the status box
            }
        } else {
            String errorMessage;
            switch (tempReading.errorCode) {
                case 1:
            ...
            state.updateStatus(errorMessage, Config::Display::COLOR_WARNING);
            drawStatusBox();
        }
    ...
    if (CoreS3.Touch.getCount() && 
        (currentMillis - state.lastTouchTime >= state.TOUCH_DEBOUNCE_MS)) {
        handleTouch();
        state.lastTouchTime = currentMillis;
    }
...
TempReading readTemperature() {
    TempReading result = {0.0f, false, 0};

    Wire.beginTransmission(0x5A);  // Address of the temperature sensor
    ...
    uint16_t rawTemp = Wire.read() | (Wire.read() << 8);
    float tempC = rawTemp * 0.02f - 273.15f;
