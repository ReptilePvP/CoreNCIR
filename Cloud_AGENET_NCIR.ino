...
void loop() {
    ...
    if (state.shouldUpdateDisplay()) {
        float temp = readTemperature();
        if (isValidTemperature(temp)) {
            cloudTemp = settings.useCelsius ? temp : celsiusToFahrenheit(temp);
            cloudMonitoring = state.isMonitoring;
            cloudStatus = state.statusMessage;
        }
    }  
            ...
            case EMISSIVITY_ADJUSTMENT:
                if (!button2State && lastButton2State) {  // Red button - decrease
                    if (settings.emissivity > Config::Emissivity::MIN) {
                        ...
                        settings.save();
                        // Set new emissivity value in sensor
                        Wire.beginTransmission(0x5A);
                        Wire.write(0x24);                   // Emissivity register address
                        ...
                        Wire.endTransmission();
                        Serial.printf("Emissivity decreased to: %.3f (raw: 0x%04X)\n", settings.emissivity, emissivityData);
                        updateDisplay();
                    }
                }
                if (!button1State && lastButton1State) {  // Blue button - increase
                    if (settings.emissivity < Config::Emissivity::MAX) {
                        ...
                        Wire.write(emissivityData >> 8);    // MSB
                        Wire.endTransmission();
                        Serial.printf("Emissivity increased to: %.3f (raw: 0x%04X)\n", settings.emissivity, emissivityData);
                        updateDisplay();
                    }
                }
                break;
    ...
    if (!currentKeyState && !state.keyPressed && (currentTime - lastKeyPressTime > debounceDelay)) {
        ...
        switch (state.menuState) {
            case MAIN_DISPLAY:
                ...
                cloudMonitoring = state.isMonitoring;  // Update cloud value
...
void onIsMonitoringChange()  {
    // Update local state when cloud value changes
    state.isMonitoring = cloudMonitoring;
    updateDisplay();
}
void handleMonitoringStateChange(bool newState, bool fromCloud) {
    ...
    if (!fromCloud) {
        // Update cloud if change was from device
        cloudMonitoring = newState;
    }
    
    // Update LED
    leds[0] = state.isMonitoring ? CRGB::Green : CRGB::Black;
    FastLED.show();
