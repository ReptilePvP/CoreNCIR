...
namespace Config {
    ...
    namespace Temperature {
        ...
        const int RANGE_MONITORING = 300;
        const int RANGE_TOO_COLD = 499;
        const int RANGE_READY = 640;
        const int RANGE_TOO_HOT = 700;
        
        // Graph range in Fahrenheit
        const int GRAPH_MIN_F = 400;
        const int GRAPH_MAX_F = 650;
    }
...
void setup() {
    ...
    CoreS3.Power.begin();
    CoreS3.Power.setExtOutput(false);  // Set to false when using Grove port
    ...
    if (abs(temp - state.currentTemp) >= 1.0 || state.currentTemp == 0) {
        ...
        if (state.isMonitoring) {
            ...
            if (!settingsMenu.isOpen) {
                drawMainDisplay();  // Update graph
                drawTemperature();
            }
        } else if (!settingsMenu.isOpen) {
            drawTemperature();  // Just update temperature display
        }
    }
...
void drawTemperature() {
    ...
    // Clear the entire temperature display area including emissivity text
